...
public class Utils {
  ...
  public static void saveToPublicDirectory(
    Uri uri,
    Context context,
    String mediaType
  ) {
  ...
  public static Uri getAppSpecificStorageUri(
    ...
    String fileType = getFileTypeFromMime(
      contentResolver.getType(sharedStorageUri)
    );
  ...
  static boolean hasPermission(final Activity activity) {
    final int writePermission = ActivityCompat.checkSelfPermission(
      activity,
  ...
  public static Uri resizeImage(Uri uri, Context context, Options options) {
    try {
      ...
      int[] newDimens = getImageDimensBasedOnConstraints(
        origDimens[0],
        origDimens[1],
        options
      );
      ...
      int orientation = originalOrientation != null
        ? Integer.parseInt(originalOrientation)
  ...
  static Bitmap getRotatedImage(Bitmap bitmap, int orientation)
    throws IOException {
  ...
  static int[] getImageDimensBasedOnConstraints(
    int origWidth,
    int origHeight,
    Options options
  ) {
  ...
  static double getFileSize(Uri uri, Context context) {
    try {
      ParcelFileDescriptor f = context
        .getContentResolver()
  ...
  static int getDuration(Uri uri, Context context) {
    ...
    int duration =
      Math.round(
        Float.parseFloat(
          m.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION)
        )
      ) /
      1000;
    try {
  ...
  static boolean shouldResizeImage(
    int origWidth,
    int origHeight,
    Options options
  ) {
  ...
  public static boolean isCameraPermissionFulfilled(
    Context context,
    Activity activity
  ) {
    try {
      String[] declaredPermissions = context
        .getPackageManager()
        .getPackageInfo(
  ...
  static WritableMap getImageResponseMap(
    Uri uri,
    Options options,
    Context context
  ) {
  ...
  static ReadableMap getResponseMap(
    List<Uri> fileUris,
    Options options,
    Context context
  )
